...
use bc_indicators::ind::no_osc::math::plus::*;
...
fn _coll_res_1()
{
    let res = plus_coll::<_, Vec<_>>(OPEN.as_slice(), CLOSE.as_slice());
    assert_eq!(res.last().unwrap(), &(OPEN_LAST + CLOSE_LAST));
    let resnan = plus_coll::<_, Vec<_>>(&[f64::NAN,], &[f64::NAN,]);
...
fn gw_div_res_1()
{
    let s = MAP_LINK::from_iter([("plus".to_string(), SETTINGS_IND{
        key: "plus".to_string(),
        used_src: vec![
            SETTINGS_USED_SRC{
                key: "open".to_string(),
    ...
    assert_eq!(rescoll["plus"].last().unwrap(), &res["plus"]);
}
