...
use bc_indicators::ind::no_osc::math::div::*;
...
fn div_coll_res_1()
{
    let res = div_coll::<_, Vec<_>>(OPEN.as_slice(), CLOSE.as_slice());
    assert_eq!(res.last().unwrap(), &(OPEN_LAST / CLOSE_LAST));
    let resnan = div_coll::<_, Vec<_>>(&[f64::NAN,], &[f64::NAN,]);
...
fn gw_div_res_1()
{
    let s = MAP_LINK::from_iter([("div".to_string(), SETTINGS_IND{
        key: "div".to_string(),
        used_src: vec![
            SETTINGS_USED_SRC{
                key: "open".to_string(),
    ...
    assert_eq!(rescoll["div"].last().unwrap(), &res["div"]);
}
