...
use bc_indicators::ind::no_osc::math::minus::*;
...
fn minus_coll_res_1()
{
    let res = minus_coll::<_, Vec<_>>(OPEN.as_slice(), CLOSE.as_slice());
    assert_eq!(res.last().unwrap(), &(OPEN_LAST - CLOSE_LAST));
    let resnan = minus_coll::<_, Vec<_>>(&[f64::NAN,], &[f64::NAN,]);
...
fn gw_div_res_1()
{
    let s = MAP_LINK::from_iter([("minus".to_string(), SETTINGS_IND{
        key: "minus".to_string(),
        used_src: vec![
            SETTINGS_USED_SRC{
                key: "open".to_string(),
    ...
    assert_eq!(rescoll["minus"].last().unwrap(), &res["minus"]);
}
